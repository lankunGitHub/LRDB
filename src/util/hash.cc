...
uint32_t Hash32(const char* data, size_t length, uint32_t seed) {
    // 简化的32位哈希函数
    uint32_t h = seed;
...
uint32_t Hash32(const Slice& slice, uint32_t seed) {
    return Hash32(slice.data(), slice.size(), seed);
}

uint64_t Hash64(const char* data, size_t length, uint64_t seed) {
    // 简化的64位哈希函数
...
uint64_t Hash64(const Slice& slice, uint64_t seed) {
    return Hash64(slice.data(), slice.size(), seed);
}
...
uint32_t CRC32(const Slice& slice) {
    return CRC32(slice.data(), slice.size());
}
...
uint32_t CityHash32(const char* data, size_t length) {
    ...
    uint32_t h = static_cast<uint32_t>(length);
...
uint64_t CityHash64(const char* data, size_t length) {
    ...
    return Hash64(data, length, static_cast<uint64_t>(length));
}
